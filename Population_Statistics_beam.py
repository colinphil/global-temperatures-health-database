...
class TransposeDateFn(beam.DoFn):
  def process(self, element):
    ...
    countryName = pop_record.get('countryName')
    countryCode = pop_record.get('countryCode')
    metric = pop_record.get('metric')
    metricCode = pop_record.get('metricCode')
...
def run():
    ...
    sql = 'SELECT * FROM kaggle2_modeled.Population_Statistics limit 20'
    ...
    query_results | 'Write input' >> WriteToText('input_population.txt')
    ...
    flatten_pcoll | 'Write output' >> WriteToText('output_population_final.txt')
    
    #create new table in BigQuery
    dataset_id = 'kaggle2_modeled'
    table_id = 'Population_Statistics_Beam'
